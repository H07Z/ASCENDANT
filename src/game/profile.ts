...
import { PETS, PET_UNLOCK_LEVEL, petBonusStats } from "./pets";
import { RNG } from "./rng";
import {
  type Item,
  type Profile,
...
/** Passive stats from the currently equipped pet (empty if none / locked). */
export function petStats(profile: Profile): Stats {
  const s = emptyStats();
  if (!profile.activePet || profile.level < PET_UNLOCK_LEVEL) return s;
  const owned = (profile.pets ?? []).find((p) => p.uid === profile.activePet);
  if (!owned) return s;
  const def = PETS[owned.defId];
  if (!def) return s;
  const bonus = petBonusStats(def, owned.level);
  (Object.keys(bonus) as (keyof Stats)[]).forEach((k) => (s[k] = bonus[k] ?? 0));
  return s;
}
